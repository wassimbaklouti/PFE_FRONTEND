...
function HouseKeepers() {
  ...
  useEffect(() => {
    fetch("http://localhost:8089/PI/handymen/houseKeeper")
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        console.log("HouseKeepers data:", data);
        // Fetch ratings for each houseKeeper after fetching their data
        const houseKeepersWithRatings = data.map(async (houseKeeper) => {
          const rating = await fetchOverallRating(houseKeeper.username);
          return { ...houseKeeper, rating };
        });
        Promise.all(houseKeepersWithRatings).then((results) => {
          setHouseKeepers(results);
          setFilteredHouseKeepers(results);
        });
      })
      .catch((error) => {
  ...
  const handleCityChange = (event) => {
    ...
    filterHouseKeepers(city, selectedRating); // Filter when the city changes
  };
  ...
  const handleRatingChange = (event) => {
    ...
    filterHouseKeepers(selectedCity, rating); // Filter when the rating changes
  };

  // Filter houseKeepers by city and rating
  const filterHouseKeepers = (city, rating) => {
    let filtered = houseKeepers;
    if (city) {
      filtered = filtered.filter((houseKeeper) => houseKeeper.city === city);
    }
    if (rating) {
      filtered = filtered.filter((houseKeeper) => houseKeeper.rating >= rating);
    }
              ...
              {filteredHouseKeepers.map((houseKeeper, index) => (
                <Grid item xs={12} md={6} xl={2} key={houseKeeper.userId || index}>
                  <DefaultProjectCard
                    ...
                    action={{
                      type: "internal",
                      route: `/pages/houseKeepers/houseKeeper-overview/${houseKeeper.userId}`,
                      color: "info",
                      label: "View Profile",
                    }}
                    authors={[
