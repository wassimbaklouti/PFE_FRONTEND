...
function Masons() {
  ...
  useEffect(() => {
    fetch("http://localhost:8089/PI/handymen/mason") // Votre endpoint API
      ...
      .then((data) => {
        console.log("Masons data:", data); // Afficher les données dans la console
        setMasons(data); // Stockage des données des plombiers
        setFilteredMasons(data); // Initialise le filtre avec tous les plombiers
      })
      .catch((error) => {
  ...
  const handleCityChange = (event) => {
    ...
    // Filter Masons based on selected city
    if (city) {
      const filtered = masons.filter((mason) => mason.city === city);
      setFilteredMasons(filtered);
    } else {
      setFilteredMasons(masons);
    }
  };
