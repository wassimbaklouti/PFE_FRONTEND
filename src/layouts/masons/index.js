...
function Masons() {
  ...
  // Fetch masons data
  useEffect(() => {
    fetch("http://localhost:8089/PI/handymen/mason")
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        console.log("Masons data:", data);
        // Fetch ratings for each mason after fetching their data
        const masonsWithRatings = data.map(async (mason) => {
          const rating = await fetchOverallRating(mason.username);
          return { ...mason, rating };
        });
        Promise.all(masonsWithRatings).then((results) => {
          setMasons(results);
          setFilteredMasons(results);
        });
      })
      .catch((error) => {
  ...
  const handleCityChange = (event) => {
    ...
    filterMasons(city, selectedRating); // Filter when the city changes
  };
  ...
  const handleRatingChange = (event) => {
    ...
    filterMasons(selectedCity, rating); // Filter when the rating changes
  };

  // Filter masons by city and rating
  const filterMasons = (city, rating) => {
    let filtered = masons;
    if (city) {
      filtered = filtered.filter((mason) => mason.city === city);
    }
    if (rating) {
      filtered = filtered.filter((mason) => mason.rating >= rating);
    }
        ...
        <MDBox mb={3}>
          ...
          <MDBox mt={3}>
            <Grid container spacing={6}>
              {filteredMasons.map((mason, index) => (
                <Grid item xs={12} md={6} xl={2} key={mason.userId || index}>
                  <DefaultProjectCard
                    image={mason.profileImageUrl || defaultImage}
                    label={`Mason #${index + 1}`}
                    ...
                    action={{
                      type: "internal",
                      route: `/pages/masons/mason-overview/${mason.userId}`,
                      color: "info",
                      label: "View Profile",
                    }}
                    authors={[
