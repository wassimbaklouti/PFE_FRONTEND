...
function Welders() {
  ...
  // Fetch welders data
  useEffect(() => {
    fetch("http://localhost:8089/PI/handymen/welder")
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        console.log("Welders data:", data);
        // Fetch ratings for each welder after fetching their data
        const weldersWithRatings = data.map(async (welder) => {
          const rating = await fetchOverallRating(welder.username);
          return { ...welder, rating };
        });
        Promise.all(weldersWithRatings).then((results) => {
          setWelders(results);
          setFilteredWelders(results);
        });
      })
      .catch((error) => {
  ...
  const handleCityChange = (event) => {
    ...
    filterWelders(city, selectedRating); // Filter when the city changes
  };
  ...
  const handleRatingChange = (event) => {
    ...
    filterWelders(selectedCity, rating); // Filter when the rating changes
  };

  // Filter welders by city and rating
  const filterWelders = (city, rating) => {
    let filtered = welders;
    if (city) {
      filtered = filtered.filter((welder) => welder.city === city);
    }
    if (rating) {
      filtered = filtered.filter((welder) => welder.rating >= rating);
    }
              ...
              {filteredWelders.map((welder, index) => (
                <Grid item xs={12} md={6} xl={2} key={welder.userId || index}>
                  <DefaultProjectCard
                    ...
                    action={{
                      type: "internal",
                      route: `/pages/welders/welder-overview/${welder.userId}`,
                      color: "info",
                      label: "View Profile",
                    }}
                    authors={[
